...
void add_space(char *);
void deck_clear(struct trump_card*,int);
void deck_cp(struct trump_card *,struct trump_card *);
bool game_start(const char *);
bool which_expreination_A_B(const char *,const char *,const char *);
void riffle_shuffle(struct trump_card *);
bool game_over(void);
void draw_a_card(struct trump_card *,int *,struct trump_card *,int *);
int add_hand(struct trump_card *,int *,struct trump_card *,int *);
int discard_and_draw(struct trump_card *,int ,struct trump_card *,int *);
void init_deck(struct trump_card *);
void game_HIGH_LOW(void);
int hand_card_sum(struct trump_card *,int);
void game_black_jack(void);
char* from_RANK_to_string(char);
char Is_this_hand_rank(struct trump_card *);
void poker_com_AI(struct trump_card *,bool *,char);
void game_poker(void);
//void discard(struct trump_card *hand,int *hand_n,int i);
//void discard_to_field(struct trump_card *field,int *field_n,struct trump_card *hand,int *hand_n,bool *select_place);
//void hand_card_simsel_sort(struct trump_card *hand,int hand_n)
//bool exists_select_place(bool *select_place,int n);
//bool can_card_discard(struct trump_card *field,int field_n,char select_count,char select_number);
...
//		
void
add_space(char *print_line){
  int i;
  for(i=strlen(print_line);i<PRINT_LINE_MAX-1;i++)
      strcat(print_line," ");
  return;
}

void
deck_clear(struct trump_card *deck,int n)
{
	int i;
	for(i=0;i<n;i++){
		(*(deck+i)).num=0;
		(*(deck+i)).mark=(MARK_TYPE)MARK_MAX;
	}
	return;

}
void
deck_cp(struct trump_card *deck_copy,struct trump_card *deck)
{
	int i;
...
void
riffle_shuffle(struct trump_card *deck)
{
	int i,j,k,temp;
	struct trump_card temp_deck[CARD_MAX];

	deck_cp(temp_deck,deck);
	for(i=0,j=CARD_MAX/2,k=0;i<CARD_MAX/2&&j<CARD_MAX;){
		for(temp=i+random(4)/*,printf("i=%d,",temp-i)*/;i<temp&&i<CARD_MAX/2;i++,k++)
		{
			(*(deck+k))=temp_deck[i];
		// printf("num=%d,\tmark=%d\n",deck[i].num,deck[i].mark);
		}
		for(temp=j+random(4)/*,printf("j=%d\n",temp-j)*/;j<temp&&j<CARD_MAX;j++,k++)
		{
			(*(deck+k))=temp_deck[j];
...
void
draw_a_card(struct trump_card *hand,int hand_n,struct trump_card *deck,int *deck_n)
{
...
int
add_hand(struct trump_card *hand,int *hand_n,struct trump_card *deck,int *deck_n)
{
	draw_a_card(hand,*hand_n,deck,deck_n);
	(*hand_n)++;

	return((*(hand+*hand_n-1)).num);
}
int
discard_and_draw(struct trump_card *hand,int i,struct trump_card *deck,int *deck_n)
{
	draw_a_card(hand,i,deck,deck_n);
	return((*(hand+i)).num);
}


void
init_deck(struct trump_card *deck)
{
	...
	for(i=0;i<MARK_MAX;i++)
		...
		(*(deck+i*NUM_MAX+j)).mark=(MARK_TYPE)i;
		}
	return;
}
...
{
	int i,n,count=0;
	struct trump_card deck[CARD_MAX];
	struct trump_card old_card,new_card;
	EXPECTATION flag=low;
	char print_line[PRINT_LINE_MAX];
	init_deck(deck);
	if(!game_start("game_HIGH_LOW")) return;
	while(1){
		for(i=0,n=random(10)+4;i<n;i++)
			riffle_shuffle(deck);

		n=0;
		draw_a_card(&new_card,0,deck,&n);
//		print_deck(deck);
		...
		while(1){
			...
			if(	(flag==high && old_card.num<=new_card.num)
			    || (flag==low && new_card.num<=old_card.num)	){
					count++;
				sprintf(print_line,"win !! =%d",count);
                                add_space(print_line);
				lcd.setCursor(0,0);
                          	//lcd.print(SPACE_MAX);
				lcd.print(print_line);
				stop_print();
			}else{
				sprintf(print_line,"lose !! win=%d",count);
                                 add_space(print_line);
				lcd.setCursor(0,0);
                        	//lcd.print(SPACE_MAX);
				lcd.print(print_line);
...
int
hand_card_sum(struct trump_card *hand,int hand_n){
	int sum,i,A_count=0;
...
game_black_jack(void)
{
	int i,n,count=0;
	struct trump_card deck[CARD_MAX];
	struct trump_card your_hand[PLAYERS_HAND_MAX_SMALL];
	struct trump_card com_hand[PLAYERS_HAND_MAX_SMALL];
...
//judge
		...
		if( BLACK_JACK<your_sum ){
			sprintf(print_line,"bust !! win=%d",count);
                        add_space(print_line);
			lcd.setCursor(0,1);
                    	//lcd.print(SPACE_MAX); 
			lcd.print(print_line);
...
char
Is_this_hand_rank(struct trump_card *hand)
{
	int i,j;
...
void
poker_com_AI(struct trump_card *hand,bool *select_place,char com_flag)
{
	int i,j;
	int kind[NUM_MAX+1];
...
//
	...
	switch(com_flag){
		...
		case no:		//this is error
								lcd.setCursor(0,0);
								lcd.print("error flag is no");exit(1);
...
//com_select
	//init
		...
		com_flag=(RANKING_HAND)Is_this_hand_rank(com_hand);
		current=6;
	//
...
//your_select
	//init
		...
		select_place[i]=false;
		while_end_flag=true;
		your_flag=(RANKING_HAND)Is_this_hand_rank(your_hand);
		current=2;
	//
...
//lcd.print("select_count!=field_n");
