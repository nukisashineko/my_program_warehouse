...
BLOCK *flower_list;
BLOCK *bord[N][N];
BLOCK muimui,butterfly,old_butterfly;
...
void BLOCK_pointer_swap(BLOCK **a,BLOCK **b)
{
	static	BLOCK temp;
	if(!*a && !*b){ /*none*/; }
	else if(!*a){ *a=*b; *b=NULL; }
	else if(!*b){ *b=*a; *a=NULL; }
	else{
		temp=**a;
		**a=**b;
		**b=temp;
	...
	for(i=0;i<N;i++){
		for(j=0;j<N;j++)
			if((c=getchar()-'0')!=0){
				...
				if(c==5){ //5==muimui
					muimui.special.x=tmpx=j;
					muimui.special.y=tmpy=i;
				}else if(c==7){//7==butterfly
					old_butterfly.special.x=butterfly.special.x=tmpx=j;
					old_butterfly.special.y=butterfly.special.y=tmpy=i;
					...
					tmpx=j;tmpy=i;temp->flower.color=(c==1)?red:yellow;
...
bool can_butterfly_move(bool temp_bord[N][N],int x,int y)
{
	BLOCK *flower_stack;
	int tmpx,tmpy;
	int tmp_old_butterfly_x=old_butterfly.special.x;
	int tmp_old_butterfly_y=old_butterfly.special.y;
	flower_stack=NULL;
	if(0<=x-1 && x+1<N &&temp_bord[x+1][y]&&bord[x-1][y]){//east
			for(tmpx=x-1;tmpx>=0&&bord[tmpx][y];tmpx--);
			if(old_butterfly.special.x!=tmpx && tmpx>=0){ 
					for(tmpx=x-1;tmpx>=0&&bord[tmpx][y];tmpx--){
						butterfly_move_above_flower(&bord[tmpx][y],&flower_stack);
					}
					old_butterfly.special.x=butterfly.special.x;
					bord[x][y]=NULL;
					butterfly.special.x=tmpx; 
					...
					butterfly.special.x=x;
				 	bord[x][y]=&butterfly;
					old_butterfly.special.x=tmp_old_butterfly_x;
	...
	if(0<=x-1 && x+1<N && temp_bord[x-1][y]&&bord[x+1][y]){//west
			...
			if(old_butterfly.special.x!=tmpx && tmpx<N){ 
					for(tmpx=x+1;tmpx<N&&bord[tmpx][y];tmpx++){
						butterfly_move_above_flower(&bord[tmpx][y],&flower_stack);
					}
					old_butterfly.special.x=butterfly.special.x;
	...
	if(0<=y-1 && y+1<N &&temp_bord[x][y+1]&&bord[x][y-1]){//south
			...
			if(old_butterfly.special.y!=tmpy && tmpy>=0){
		 			for(tmpy=y-1;tmpy>=0&&bord[x][tmpy];tmpy--){
						butterfly_move_above_flower(&bord[x][tmpy],&flower_stack);
					}
					old_butterfly.special.y=butterfly.special.y;
					bord[x][y]=NULL; 
					butterfly.special.y=tmpy;
					...
					butterfly.special.y=y;
				 	bord[x][y]=&butterfly;
					old_butterfly.special.y=tmp_old_butterfly_y;
	...
	if(0<=y-1 && y+1<N &&temp_bord[x][y-1]&&bord[x][y+1]){//north
			...
			if(old_butterfly.special.y!=tmpy && tmpy<N){ 
					for(tmpy=y+1;tmpy<N&&bord[x][tmpy];tmpy++){
						butterfly_move_above_flower(&bord[x][tmpy],&flower_stack);
					}
					old_butterfly.special.y=butterfly.special.y;
...
//	for(i=0;i<N;i++){ temp_bord[i]=(bool*)malloc(sizeof(bool)*N); }
	...
	if(	(0<=butterfly.special.x && butterfly.special.x<8) && (0<=butterfly.special.y && butterfly.special.y<8)
		&& can_butterfly_move(temp_bord,butterfly.special.x,butterfly.special.y)){ return(true); }
